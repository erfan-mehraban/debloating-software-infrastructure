...
def process(rootpid, traces_prefix, cwd='/', iscontainerroot=True):
    ...
    def parse_helper(pid, parser):
        ...
        with open(trace) as f:
            parser.parse(f)
    def dfs_helper(parser):
        for record in parser.exec_records:
            for child, cwd in record.children:
                parsers[child] = StraceParser(cwd, record.exe, record.argv,
                        record.envp)
                parse_helper(child, parsers[child])
                dfs_helper(parsers[child])
    parse_helper(rootpid, parsers[rootpid])
    dfs_helper(parsers[rootpid])

    return parsers

def existing_files(rootpid, traces_prefix, cwd='/'):
    trace_res = process(rootpid, traces_prefix, cwd)
    return {file for files in trace_res.values() for file in files}
